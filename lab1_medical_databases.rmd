...
NoShow if the Patient did not show up scheduled appointment (Yes/No)
...
2. Older Age (may forget about appointment or may not be able to physically get there)
3. Mental health and addiction issues (may not want to go there (embarassment/stigma) or may not have resourses/transportation to get there)
...
1. Distance from place of residence to appointment site
2. Length of stay in hospital/Admissions to hospital
...
```{r}
raw.data <- data %>% filter()
```

While there are two people that are 115 years old, we cannot actually tell if this is an impossible age. One of the individuals does have hypertension, which can be mananged using medication, but it may not be biologically plausible for an individual to live to 115 years while having hypertension. It also says that the individual did recieve a text message reminder, in this day and age alot of individuals do have cell phones, an individual that is 115 years old may not have a cell phone. This age may be an imputation error or it could be correct, but these should be considered with caution as they are borderline biologically implausible. These individuals will be kept in the data set but when interpreting results, there should be considerations for these individuals.
...
The maximum number of appointments from the same patient (822145925426128) is 88.They did not show up to alot of their appointments though. 
...
SMSRecieved and ScheduledDate are most strongly correlated with NoShow, even though these are not above 0.5 or below -0.5 they have the highest values on the column.
...
Yes, there is an issue with PatientID/AppointmentID being included in this plot, as they are variables that are supposed to be random identifiers so they should not have any correlation.

For example a singular patient (PatientID) could have more than 1 appointment, which would include more than 1 AppointmentID. Therefore AppointmentID could only ever have one PatientID associated to it, but a PatientID could have more than one AppointmentID.
...
From the plot it does look like SMS reminders increase the chance of someone not attending an appointment. This could be the case because individuals who have same day appointments may not be receiving a text message reminder, and they do show up. This could also be biased because older individuals may be less likely to have cell phones, and therefore may not be able to recieve text messages. 
...
A possible reason for differences in attendance rates across neighbourhoods could be differences in socioeconomic status or distance from appointment site. 
...
In the graph, it does show that there are lag days that are less than zero. This means that the scheduled date was a later date than when the appointment date was. This could be an error when entering the dates and interpreting the days and months in reverse.
...
Based on the exploratory data analysis, this 
...
```{r}
crt.grid <- expand.grid(mtry = c(2,4,6), eta=c(0.05),
                       max_depth=c(4),colsample_bytree=1,
                       subsample=1, nrounds=500, gamma=0, min_child_weight=5)
crt.model <- train(NoShow ~ .,data=train, method="rf",metric="ROC",
                  tuneGrid=crt.grid, trControl=fit.control)

crt.pred <- predict(crt.model, newdata=test)
crt.probs <- predict(crt.model, newdata=test, type="prob")
```
