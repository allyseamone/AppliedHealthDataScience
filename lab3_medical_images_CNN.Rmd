...
# zip_file <- "/Users/allyseamone/Documents/School/Health Data Science/AppliedHealthDataScience/data/lab3_chest_xray.zip"
# 
...
Pneumonia is an infection in the lungs and causes inflammation of the sacs in the lungs as they fill with fluid or pus. This would be be able to be seen on a scan/X-ray of the chest/lungs.

The point/benefit of being able to identify it from X-rays automatically is that individuals can get on an antibiotic medication earlier and be able to start recovering sooner. This would also be helpful to reduce the number of appointments with a physician and optimize their time when meeting with patients. To identify pneumonia earlier would be especially important when diagnosing patients with lung/respiratory diseases such as asthma, COPD or cystic fibrosis, etc.
...
The file names do tell if each x-ray/image shows pneumonia or normal lungs. This may make predicting pneumonia more challenging because there is already a diagnosis present in the file name.
...
The size of the images to all be the same and labels for the diagnosis (pneumonia vs. normal).
...
If the dataset was not balanced, you could reduce the number in the oversampled group to match the other group, which would balance the dataset. This technique would be called undersampling the dataset. 
...
Based on the training and test accuracy, the model is not managing to classify X-rays into pneumonia vs normal. This is seen through the second plot as the train accuracy does increase as the epoch increases, and the test accuracy stays constant as the epoch increases. Even though there is an increase in accuracy, this is quite small from 0.49... to 0.504... which probably does not make too much of a difference in a dataset that includes around 600 images.

I think that the small sample size contributes to this. Since there are not examples of what all types of pneumonia x-rays could look like, some misclassification could occur depending on what the x-ray is showing. If the case of pneumonia is in an earlier stage then the x-ray may look similar to normal lungs compared to pneumonia that has been developing for longer.

Also the complexity of the model or that the training process may need adjustments to improve test and training accuracy. 
...
The suggestions to solve this problem is to increase the sample of images in the training to include a more diverse range of pneumonia x-rays at different stages/days of infection, or increase epochs.
...
The potential drawbacks or disadvantages of data augmentation is that biases can carry through from the original data set to the augmented data set. Another disadvantage is that the data sets need to be evaluated for quality purposes.
...
ImageNet is a database that has organized images based on the WordNet hierarchy (nouns only), from this hierarchy, each node is shown by thousands of images. ImageNet is useful in computer vision and deep learning models.

ImageNet Large Scale Visual Recognition Challenge 2012 is a competition to take images from the large ImageNet data set as the training data and estimate their content. There are 10 million images in the data set and over 10 thousand classes. The purpose of this competition is to estimate content for annotation purposes and retrieval of the images. The main goal here is to identify the most prominent object in the images.

This may help us to identify lungs that have pneumonia by identifying the lungs with inflammation due to the infection. ImageNet would also be helpful as there is a huge amount of training data and this data set has quite a small sample of around 600 images in the training set. 

ImageNet is also helpful since it is very effective in transfer learning scenarios. ImageNet could be helpful here based on the transfer learning scenarios.
...
Using this architecture in the practical assignment can help improve the results by increasing the efficiency of the model. This architecture is smaller and faster. MobileNet is also trained in many different tasks, and it was shown to be quite effective in all tasks. MobileNet also has depthwise separable convolution, which is better than regular convolution. 
...
MobileNetV2 has 3.4 million parameters. This network uses depthwise separable convolution (made from 2 operations: depthwise convolution and pointwise convolution) compared to other networks that use regular convolution.

In the following you can see an example of how we can load pre-trained models in our codes. 

This did not work for me, I am not sure if I did #10 right.
...
With the added layer (self$fc3) in the nn_module and 2 epochs, this does not look to perform much better than our original network architecture. From the graph below, the train accuracy just decreases as the 2 epochs occur and the test loss also decreases as the epochs occur. From either epoch, the accuracy stays around 0.5 which is very similar to the original network architecture.
